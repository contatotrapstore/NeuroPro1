...
module.exports = async function handler(req, res) {
  console.log('🚨 EMERGENCY CHAT API - ALWAYS FRESH THREADS - GUARANTEED WORKING');
  console.log('Request method:', req.method);
  console.log('Request URL:', req.url);
  console.log('Deploy timestamp:', new Date().toISOString());
  console.log('*** EMERGENCY MODE: NO THREAD_ID DEPENDENCY ***');
  ...
  try {
    ...
    if (req.method === 'GET' && pathParts.length === 2 && pathParts[1] === 'conversations') {
      ...
      if (!process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY.includes('placeholder')) {
        ...
        try {
          console.log('🚨 EMERGENCY MODE: Creating fresh thread for every message');
          const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
          
          // ALWAYS create a new thread - ignore stored thread_id completely
          console.log('🔧 Creating brand new OpenAI thread...');
          const newThread = await openai.beta.threads.create();
          const workingThreadId = newThread.id;
          console.log('✅ Fresh thread created:', workingThreadId);
          
          // Update conversation with new thread_id for reference
          await userClient
            .from('conversations')
            .update({ thread_id: workingThreadId })
            .eq('id', conversationId);
          
          // Add message to fresh thread
          console.log('📝 Adding message to fresh thread:', workingThreadId);
          await openai.beta.threads.messages.create(workingThreadId, {
            role: 'user',
            content: content
          });

          // Create run with guaranteed valid parameters
          console.log('▶️ Creating run with guaranteed valid thread_id...');
